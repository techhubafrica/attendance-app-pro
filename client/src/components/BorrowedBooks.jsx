...
const BorrowedBooks = () => {
  ...
  const overdueBooks =
    borrowedBooks?.filter(
      (loan) => loan.status === "borrowed" && isOverdue(loan.dueDate)
    ) || [];

  const nearlyDueBooks =
    borrowedBooks?.filter(
      (loan) =>
      ...
      {(overdueBooks.length > 0 || nearlyDueBooks.length > 0) && (
        <div className="px-6 space-y-3 bg-slate-50 py-3">
          {overdueBooks.length > 0 && (
            <Alert
              variant="destructive"
              className="border-red-600 bg-red-50 shadow-sm"
            >
              ...
              <AlertDescription className="text-red-600">
                You have {overdueBooks.length} overdue{" "}
                {overdueBooks.length === 1 ? "book" : "books"}. Please return{" "}
                {overdueBooks.length === 1 ? "it" : "them"} soon to avoid
                additional fees.
              </AlertDescription>
            </Alert>
          )}
          ...
          {nearlyDueBooks.length > 0 && (
            <Alert
              variant="outline"
              className="border-amber-500 bg-amber-50 shadow-sm"
            >
              ...
              <AlertDescription className="text-amber-600">
                You have {nearlyDueBooks.length}{" "}
                {nearlyDueBooks.length === 1 ? "book" : "books"} due within the
                next 3 days.
              </AlertDescription>
            </Alert>
          )}
      ...
      {overdueBooks.length > 0 && (
        <CardFooter className="border-t bg-red-50 text-sm text-red-700 py-3">
          <p className="flex items-center">
            ...
            Please note: Overdue books may incur additional fees. Contact the
            library if you need assistance.
          </p>
        </CardFooter>
      )}
    </Card>
