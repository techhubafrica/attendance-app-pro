...
import { RefreshCw, Users, Clock, Calendar, UserCheck, UserX } from 'lucide-react';
import { getAllAttendance, getEmployeeAttendance } from '@/redux/slices/attendanceSlice';
...
const AdminAttendance = () => {
  ...
  const { employeeAttendance, loading } = useAppSelector((state) => state.attendance);
  ...
  const getAttendanceStats = () => {
    ...
    const presentToday = employeeAttendance.filter(record => 
  ...
  return (
    <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8 py-8 min-h-screen mt-16">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-blue-800 mb-1">Attendance Management</h1>
        ...
        <Card className="border-blue-200 shadow-md bg-gradient-to-r from-green-50 to-green-100">
          <CardContent className="flex items-center p-6">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-green-800">{stats.presentToday}</h3>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-blue-200 shadow-md bg-gradient-to-r from-red-50 to-red-100">
          <CardContent className="flex items-center p-6">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-red-800">{stats.absentToday}</h3>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <Table>
            <TableHeader className="bg-gradient-to-r from-blue-100 to-green-100">
              <TableRow>
                ...
                <TableHead className="text-blue-800 font-semibold">Check In</TableHead>
                <TableHead className="text-blue-800 font-semibold">Check Out</TableHead>
                <TableHead className="text-blue-800 font-semibold">Working Hours</TableHead>
                <TableHead className="text-blue-800 font-semibold">Breaks</TableHead>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-10">
                    ...
                    <div className="mt-2 text-sm text-gray-500">Loading attendance records...</div>
                  </TableCell>
                </TableRow>
                ...
                employeeAttendance.map((record, index) => (
                  <TableRow key={index} className="hover:bg-blue-50 transition-colors">
                    <TableCell className="font-medium">
                      {record.user?.user?.name || `Employee #${record.user?._id || 'Unknown'}`}
                    </TableCell>
                    <TableCell>
