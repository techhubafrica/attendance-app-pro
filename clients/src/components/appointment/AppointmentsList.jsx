...
import { CheckCircle, X, Calendar, Building, User, FileText } from 'lucide-react';
import { checkInAppointment, checkOutAppointment } from '@/redux/actions/appointmentActions';

const AppointmentsList = () => {
  const dispatch = useDispatch();
  const { userAppointments, isLoading } = useSelector(state => state.appointments);
  ...
  if (!userAppointments || userAppointments.length === 0) {
    return (
      <div className="text-center py-12 bg-blue-50 rounded-lg border border-blue-200">
        <h3 className="text-lg font-medium text-blue-700">No appointments found</h3>
      ...
      <Table>
        ...
        <TableBody>
          {userAppointments.map((appointment, index) => (
            <TableRow 
              key={appointment._id}
              className={`${index % 2 === 0 ? 'bg-blue-50 bg-opacity-30' : 'bg-green-50 bg-opacity-30'} hover:bg-blue-100 hover:bg-opacity-40 transition-colors`}
            >
